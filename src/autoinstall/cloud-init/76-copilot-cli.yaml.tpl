...
runcmd:
  ...
  # Write Copilot CLI configuration with OAuth token
  - |
...
{% if copilot_cli.auth.gh_token is defined %}
  ...
  - |
    cat > /home/{{ identity.username }}/.bashrc.d/copilot-cli.sh << 'COPILOT_BASHRC_EOF'
    # GitHub Copilot CLI token (fallback)
    export GH_TOKEN="{{ copilot_cli.auth.gh_token }}"
    COPILOT_BASHRC_EOF
  - chmod 600 /home/{{ identity.username }}/.bashrc.d/copilot-cli.sh
  - chown {{ identity.username }}:{{ identity.username }} /home/{{ identity.username }}/.bashrc.d/copilot-cli.sh
{% endif %}
