...
{% endif %}
      ...
      find_opencode_model() {
        local model="$1"
        local filter="${2:-}"
        local models_list=$(opencode models 2>/dev/null)
...
{% if opencode.enabled | default(false) %}
        ...
        # Derive model from Claude Code or Copilot CLI config, validate with opencode models
        local oc_model=""
{% if claude_code.model is defined %}
        # Try Claude Code model first (anthropic/*)
        local cc_model_id="{{ claude_code.model }}"
        local cc_candidate="anthropic/$cc_model_id"
        local cc_matched=$(find_opencode_model "$cc_candidate" "anthropic")
        if [ -n "$cc_matched" ]; then
          oc_model="$cc_matched"
          log "Using Claude Code model: $oc_model"
        fi
{% endif %}
{% if copilot_cli.model is defined %}
        # Try Copilot CLI model if Claude model not valid (github-copilot/*)
        if [ -z "$oc_model" ]; then
          local cp_model_id="{{ copilot_cli.model }}"
          local cp_candidate="github-copilot/$cp_model_id"
          local cp_matched=$(find_opencode_model "$cp_candidate" "github-copilot")
          if [ -n "$cp_matched" ]; then
            oc_model="$cp_matched"
            log "Using Copilot CLI model: $oc_model"
          fi
        fi
{% endif %}
        # Fallback to anthropic/claude-haiku-4-5
        if [ -z "$oc_model" ]; then
          oc_model="anthropic/claude-haiku-4-5"
          log "Using fallback model: $oc_model"
        fi

        ai_output=$(HOME="$OPENCODE_HOME" opencode --model "$oc_model" run "$prompt" 2>/dev/null)
        ai_status=$?
        if [ -n "$ai_output" ]; then
...
{% if claude_code.enabled | default(false) %}
        ...
        # Derive and validate model using opencode models (anthropic/*)
        local cc_model=""
{% if claude_code.model is defined %}
        local cc_model_id="{{ claude_code.model }}"
        local cc_candidate="anthropic/$cc_model_id"
        ...
        if [ -n "$cc_matched" ]; then
          ...
          cc_model="${cc_matched#anthropic/}"
          log "Using validated model: $cc_model"
        fi
{% endif %}
        # Fallback to claude-haiku-4-5 if model not valid
        if [ -z "$cc_model" ]; then
          cc_model="claude-haiku-4-5"
...
{% if copilot_cli.enabled | default(false) %}
        ...
        # Derive and validate model using copilot error output or opencode models (github-copilot/*)
        local cp_model=""
{% if copilot_cli.model is defined %}
        local cp_model_id="{{ copilot_cli.model }}"
        # First try to validate via copilot --model fakemodel error output (with fuzzy matching)
        local cp_matched=$(find_copilot_model "$cp_model_id")
        if [ -n "$cp_matched" ]; then
          cp_model="$cp_matched"
          log "Using validated Copilot model: $cp_model"
        else
          # Fallback to opencode models validation
          local cp_candidate="github-copilot/$cp_model_id"
          local oc_matched=$(find_opencode_model "$cp_candidate" "github-copilot")
          if [ -n "$oc_matched" ]; then
            # Extract just the model name (after provider/)
            cp_model="${oc_matched#github-copilot/}"
            log "Using OpenCode-validated Copilot model: $cp_model"
          fi
        fi
{% endif %}
        # Fallback to claude-haiku-4.5 if model not valid
        if [ -z "$cp_model" ]; then
          cp_model="claude-haiku-4.5"
        ...
        if [ -n "$ai_output" ]; then
          if [ $ai_status -eq 0 ]; then
            ...
            partial_outputs+="[Copilot CLI (exit $ai_status)]${NL}${ai_output}${NL}${NL}"
          fi
        fi
{% endif %}
