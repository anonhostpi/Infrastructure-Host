...
# Config caching module - ensures Get-TestConfig is called only once
New-Module -Name Test-Config -ScriptBlock {
    $cache = @{ Config = $null }
    ...
    function Get-TestConfig {
        if ($null -eq $cache.Config) {
            $cache.Config = Build-TestConfig
        }
        return $cache.Config
    }

    Export-ModuleMember -Function Get-TestConfig
} | Import-Module -Force